...
base_path = "/local/scratch/datasets/FullbodySCT/SynthRAD2025/task1/"

path_for_norm_CT = base_path + "before_temp/CT_reg"
path_for_norm_MR = base_path + "before_temp/MR"
#body mask
path_new_masks = base_path + "before_temp/masks"

path_excel = base_path + "excel/data_CT_MR_TEMP_second_paper.xlsx"
path_excel_resampling = base_path + "excel/resampling_TEMP_second_paper_air_OR.xlsx"
...
def main():
    ...
    for path_r in [path_for_norm_CT, path_for_norm_MR, path_new_masks]:
    ...
    if not os.path.exists(path_excel):
        print(f"Excel not found: {path_excel}")
        return
    ...
    for index, line in df_all.iterrows():
        ## check preconditions (for each loop)
        if not (line.TreatmentDay in accepted_tr_days and 
                line.ModalityFolder in accepted_modal_folders and 
                str(line.PathNIFTI).strip() !="") :
            print("Skip patient {}".format(line.Patient))
            continue
