...
for i = 1:numOfSlices
    ...
    tmpDicomInfo = matRad_safeDicomInfo(obj.importFiles.ct{i, 1}, matRad_cfg);
    ...
    if isfield(tmpDicomInfo,'PatientPosition') && ~isempty(tmpDicomInfo.PatientPosition)
        obj.importCT.ctInfo(i).PatientPosition = tmpDicomInfo.PatientPosition;
    else
        % Some synthetic/converted DICOM series omit PatientPosition.
        % Fall back to HFS so orientation checks can proceed.
        obj.importCT.ctInfo(i).PatientPosition = 'HFS';
    end
...
function info = matRad_safeDicomInfo(filename, matRad_cfg)
