...
structInfo = matRad_safeDicomInfoRtss(obj.importFiles.rtss{1}, matRad_cfg);
...
function structInfo = matRad_safeDicomInfoRtss(filename, matRad_cfg)
if matRad_cfg.isOctave || verLessThan('matlab','9')
    structInfo = dicominfo(filename);
    return;
end

% Try the strict read first; fall back for older MATLAB versions.
try
