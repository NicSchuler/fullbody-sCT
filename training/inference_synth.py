#!/usr/bin/env python
"""
Inference script for MR-to-CT synthesis using pix2pix or cycleGAN models.

This is a simplified version for inference-only mode:
- Uses single_nifti dataset mode (MR only, no paired CT)
...
Usage:
    python inference_synth.py \
        ...
        --name pix2pix_synthrad_allregions_final \
        --checkpoints_dir /path/to/checkpoints \
        --epoch 50 \
        --results_dir /path/to/output \
        --model pix2pix  # or cycle_gan

This script:
    1. Loads MR slices from --dataroot
    2. Runs pix2pix or cycleGAN model inference
...
def main():
    ...
    print("=" * 60)
    print(f"MR-to-CT Inference ({model_type} Model)")
    print("=" * 60)
    ...
    for i, data in enumerate(tqdm(dataset, desc="Inference")):
        # Both pix2pix and cycle_gan models expect 'B' key in set_input
        # Provide dummy B for inference (not used, but required)
        if 'B' not in data:
        ...
        # Save fake CT (output key is 'fake_B' for both pix2pix and cycle_gan)
